...
class CatalogSyncService:
    ...
    async def upsert_uploaded_episode(self, *, anime_title: str, episode_number: int | None, quality: str, file_format: str, telegram_file_id: str, telegram_message_id: int, file_size: int, duration: int | None, metadata: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        episode = await self.mongo.db.episodes.find_one_and_update(episode_key, {"$set": {"anime_id": anime_id, "episode_number": episode_number, "quality": quality, "format": file_format, "telegram_file_id": telegram_file_id, "telegram_message_id": telegram_message_id, "telegram_channel_id": self.channel_id, "file_size": file_size, "duration": duration, "updated_at": now, **(metadata or {})}, "$setOnInsert": {"stream_slug": f"{anime_id}-{episode_number or 'unknown'}-{quality}", "created_at": now}}, upsert=True, return_document=ReturnDocument.AFTER)
        return episode
