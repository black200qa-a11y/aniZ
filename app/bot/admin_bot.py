    ...
    async def start(self) -> None:
        ...
        async def callback_handler(_, callback: CallbackQuery):
            ...
            if action == "status":
                ...
                await callback.message.edit_text(text, reply_markup=self._menu())
            elif action == "recent":
                ...
                await callback.message.edit_text("🔄 Services use supervisor/container restart policy. Cache/state is preserved; use your process manager to restart safely.", reply_markup=self._menu())
        ...
        async def manual_handler(_, message: Message):
            ...
            result = await self.mongo.db.manual_jobs.insert_one({"source": source, "status": "PENDING"})
