...
def load_values(path: Path) -> dict[str, str]:
    ...
    values.update({key: value for key, value in os.environ.items() if key in values or key in REQUIRED})
    return values


REQUIRED = {"API_ID", "API_HASH", "STRING_SESSION", "TG_CHANNEL_ID", "MONGODB_URI", "DATABASE_NAME", "ARIA2_HOST", "ARIA2_PORT", "ARIA2_SECRET", "API_PORT", "STREAM_CHUNK_SIZE", "MAX_CONCURRENT_STREAMS", "NYAA_CHECK_INTERVAL_MINUTES", "QUALITY_FILTERS", "RELEASE_GROUPS"}
...
def validate_config(values: dict[str, str]) -> tuple[bool, str]:
    ...
    try:
        if int(values["API_ID"]) <= 0 or int(values["TG_CHANNEL_ID"]) == 0:
            raise ValueError("Telegram IDs are invalid")
        for key in ("ARIA2_PORT", "API_PORT", "STREAM_CHUNK_SIZE", "MAX_CONCURRENT_STREAMS", "NYAA_CHECK_INTERVAL_MINUTES"):
            if int(values[key]) <= 0:
